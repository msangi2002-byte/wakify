...
public class AgentService {
    ...
    public BusinessResponse activateBusiness(UUID agentUserId, BusinessActivationRequest request) {
        ...
        if (request.getOwnerId() != null) {
            ...
            if (request.getOwnerPhone() == null || request.getOwnerName() == null) {
                throw new BadRequestException("Owner phone and name are required for new business owner");
            }

            // Check if phone already exists
            if (userRepository.existsByPhone(request.getOwnerPhone())) {
                throw new BadRequestException("Phone number already registered. Use existing user ID instead.");
            }
            ...
            owner = User.builder()
                    ...
                    .password(passwordEncoder.encode("temp" + System.currentTimeMillis())) // Temp password
                    .role(Role.BUSINESS)
                    .isVerified(false)
                    .isActive(true)
                    .build();
    ...
    public PagedResponse<CommissionResponse> getAgentCommissions(UUID agentUserId, int page, int size) {
        ...
        Pageable pageable = PageRequest.of(page, size);
        Page<Commission> commissions = commissionRepository.findByAgentIdOrderByCreatedAtDesc(agent.getId(), pageable);
    ...
    public void confirmPayment(String transactionId) {
        ...
        if (payment.getType() == PaymentType.AGENT_REGISTRATION) {
            ...
            Agent agent = agentRepository.findByUserId(payment.getUser().getId())
                    .orElseThrow(() -> new ResourceNotFoundException("Agent not found for payment"));
            ...
            Business business = businessRepository.findByOwnerId(payment.getUser().getId())
                    .orElseThrow(() -> new ResourceNotFoundException("Business not found for payment"));
    ...
    private CommissionResponse mapToCommissionResponse(Commission commission) {
        return CommissionResponse.builder()
                ...
                .businessName(commission.getBusiness() != null ? commission.getBusiness().getName() : null)
