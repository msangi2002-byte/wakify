...
public interface ProductRepository extends JpaRepository<Product, UUID> {
    ...
    /** Marketplace listing ordered by supplier performance (rating) and engagement (views). */
    @Query("SELECT p FROM Product p JOIN p.business b WHERE p.isActive = true ORDER BY COALESCE(b.rating, 0) DESC, COALESCE(p.viewsCount, 0) DESC, p.createdAt DESC")
